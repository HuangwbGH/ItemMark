...
状态：当前技术设计基线，已吸收 2026-05-09 的确认项
...
- [8. 同步设计](#8-同步设计)
- [9. U8 台账设计](#9-u8-台账设计)
- [9.1 后台权限策略](#91-后台权限策略)
- [10. Docker 和 Nginx 设计](#10-docker-和-nginx-设计)
- [11. 旧项目资料迁移策略](#11-旧项目资料迁移策略)
- [11.1 相对路径约束](#111-相对路径约束)
- [12. 实施步骤](#12-实施步骤)
- [13. 验证命令](#13-验证命令)
- [14. 已识别风险](#14-已识别风险)
...
- `get_label_config(module_key, template_key)`
- `list_label_templates(module_key)`
- `get_visible_fields(module_key)`
- `get_template_path(module_key)`

配置读取规则：

- `material-info` 模块打印页支持 `template` 查询参数；未传或传 `default` 时读取 `label_config.xlsx`。
- `material-info` 模块可选模板来自 `config/templates/material-info/` 下的 `.xlsx` 文件，默认模板 key 为 `default`，其他模板 key 使用文件名去掉 `.xlsx`。
- Excel `配置` 工作表支持 `模板名称` 和 `布局类型`。`布局类型=standard` 使用原标签布局，`布局类型=split_2` 使用一张纸两个信息区的辅料双联布局。
- 辅料双联布局使用原打印纸宽高作为一张物理标签纸，内部上下分成两个信息区；批量打印时每两个已选物料组成一张纸，单张打印时同一物料上下复制成两个信息区。
- `配置` 工作表支持 `名称字体`、`名称字号`、`字段字体`、`字段字号`、`字段标签字号`、`字段内容最多行数`、`颜色最多行数`、`备注最多行数`、`标签列宽`、`底部字体`、`底部字号`。
- 辅料双联布局中 `color_desc` 字段使用 `颜色最多行数` 单独控制截断行数，避免长颜色描述过早省略。
- 辅料双联布局中“物料编码”“颜色”等固定字段标题使用 `字段标签字号` 控制文字大小，使用 `标签列宽` 控制标题列宽。
- 字体配置支持单个字体名或用英文逗号分隔的字体列表，服务端读取后输出给打印页 CSS。
- 模板未配置字体时使用默认字体栈 `Microsoft YaHei, PingFang SC, Arial, sans-serif`。
- 单张和批量打印页使用固定标签高度，页脚不随字段内容被挤到下一页。
- 普通字段按 `字段内容最多行数` 做行数截断，备注字段按 `备注最多行数` 做行数截断，避免任一字段过长时单张标签内容跨页；设置为 `0` 时不限制对应字段截断。
- 单张和批量打印页的底部信息颜色使用 `#111`，与信息字段正文保持一致。
- 标签宽度、最小高度、内边距、二维码大小、每行标签数、标签间距、每页标签数等布局配置仍沿用原逻辑。
- 批量打印在 `每页标签数 = 1` 时打印样式去除标签间距，并仅对非最后一张标签强制换页，避免标签之间产生空白页。

实施策略：

- 对外物料信息模块直接复制项目 1 的 `config/label_config.xlsx`。
- 对外物料信息模块内置 `accessory_split_2.xlsx` 辅料双联模板。
...
0. 编码前按根目录 `AGENTS.md` 写明假设、成功标准和验证方式。
