...
CONFIG_ROWS = [
    ...
    ("信息字段", "字段标签字号",    8.5, 8.5,"字段标签文字（如物料编码、颜色）的字体大小", "pt"),
    ("信息字段", "字段内容最多行数", 2,   2, "普通字段内容超出行数时自动截断，0 = 不限制", "行"),
    ("信息字段", "颜色最多行数",    4,   4, "颜色字段内容超出行数时自动截断，0 = 不限制", "行"),
    ("信息字段", "备注最多行数",    4,   4, "备注内容超出行数时自动截断，0 = 不限制", "行"),
