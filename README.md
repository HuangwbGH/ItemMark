...
- 字段字体。
- 字段字号。
- 字段标签字号：字段固定标题文字大小，例如辅料双联模板中的“物料编码”“颜色”。
- 字段内容最多行数：普通字段过长时按配置截断，避免字段内容把页脚挤到下一页；设置为 `0` 时不限制。
- 颜色最多行数：辅料双联模板中颜色字段过长时按配置截断；设置为 `0` 时不限制。
- 备注最多行数：备注过长时按配置截断，避免单张标签被撑到下一页；设置为 `0` 时不限制。
- 标签列宽：字段固定标题列宽，例如辅料双联模板中的“物料编码”“颜色”所在列。
- 二维码大小。
- 每行标签数。
- 每页标签数。
- 批量打印分页：每页 1 张标签时自动去除打印间距，并且最后一张后不再强制换页，避免空白页。
- 显示字段和显示顺序。
- 标签底部是否显示。
- 标签底部字体。
- 标签底部字号。
- 标签底部文字颜色与信息字段正文保持一致，确保打印预览和打印输出清晰。
- 标签底部左侧内容。
- 标签底部右侧内容。

底部内容支持占位符：

字体配置项支持在模板中填写单个字体名或用英文逗号分隔的字体列表，例如 `Microsoft YaHei, PingFang SC, Arial, sans-serif`。未配置时使用系统默认标签字体，不影响现有标签宽度、高度、间距和分页布局。
