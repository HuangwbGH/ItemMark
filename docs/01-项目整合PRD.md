...
状态：当前产品需求基线，已吸收 2026-05-09 的确认项
...
1. 把两个旧项目整合为一个 ItemMark 项目，所有相关文件放在项目根目录下。
...
8. 程序实现必须遵循根目录 `AGENTS.md`：实现前明确假设和成功标准，优先简单方案，只做必要修改，完成后验证。
...
- 对外物料信息模块支持配置多个打印模板，用户打印标签时可以选择指定模板。
- 对外物料信息模块默认模板继续使用 `config/templates/material-info/label_config.xlsx`，同目录下其他 `.xlsx` 文件作为可选模板。
- 对外物料信息模块新增辅料双联模板：在原本打印纸大小内上下打印两个物料信息区，信息区包含“物料名称、物料编码、颜色、二维码”。
- 辅料双联模板批量打印时每张纸上下排两个已选物料；已选数量为奇数时，最后一个信息区留空。
- 辅料双联模板单张打印时，同一物料上下打印两份信息区。
- 对外物料信息模块优先复制旧项目 1 已调试好的模板。
- 武平模块使用旧项目 2 的模板逻辑，默认字段包含“单位”，二维码路径指向 `/wuping/material/{code}`。
- 标签模板必须支持配置名称区、信息字段区、底部信息区的字体和字号；默认值保持现有打印效果。
- 底部信息文字颜色必须与信息字段正文保持一致，避免打印预览和打印输出中底部信息过浅。
- 信息字段或备注内容过长时，打印页必须支持按配置行数截断，避免页脚或单张标签内容被撑到下一页；完整内容仍可通过扫码详情查看。
- 新增字体配置不得改变既有标签宽度、高度、间距、每行数量和分页等布局能力。
...
- 批量选择。
- 批量打印。
- 读取当前模块的标签配置，包括标签尺寸、布局、字段显示、字体和字号。
...
文档、配置、Docker Compose 和运行时代码中不得写入开发机或服务器的本机绝对路径。
...
5. 两个模块可以分别配置标签模板。
6. 任一信息字段或备注内容过长时，单张和批量打印预览不应把同一张标签或页脚拆到下一页。
7. 批量打印每页 1 张标签时，标签之间不应出现空白页。
8. 后台页面可以查看同步记录、修改同步计划、手动触发同步。
9. 首次运行可以重新同步数据。
10. Docker Compose 可在 Linux OS 启动。
