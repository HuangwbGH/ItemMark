...
accessory_split_2.xlsx -> 配置工作表包含“颜色最多行数”，默认值为 4。
GET /material-info/print/1?template=accessory_split_2 -> 页面包含 --color-lines: 4 和 .split-row.field-color_desc。
accessory_split_2.xlsx -> 配置工作表包含“字段标签字号”，默认值为 7，并包含“标签列宽”。
GET /material-info/print/1?template=accessory_split_2 -> 页面包含 --field-label-fs 和 grid-template-columns: var(--field-kw) 1fr。
GET /material-info/batch/print?codes=1&template=accessory_split_2 -> 页面包含 --field-label-fs 和 grid-template-columns: var(--field-kw) 1fr。
```
